	...
	def Execute(kimin):
		...
		try:
			if kimin.parameter['method'].lower() == 'get':
				respon = requests.request(kimin.parameter['method'], kimin.parameter['url'], headers=kimin.parameter['header'])
			elif kimin.parameter['method'].lower() == 'post' or 'patch':
				if kimin.parameter['data_type'].lower() == 'json':
					respon = requests.request(kimin.parameter['method'], kimin.parameter['url'], headers=kimin.parameter['header'], json=kimin.parameter['data'])
				elif kimin.parameter['data_type'].lower() == 'form':
					respon = requests.request(kimin.parameter['method'], kimin.parameter['url'], headers=kimin.parameter['header'], data=kimin.parameter['data'])
